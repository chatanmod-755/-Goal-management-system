...
public class Goal_edit_month_progress extends HttpServlet {
    public void select_month_judge(String goal_id,HttpServletRequest request, HttpServletResponse response){//目標を表示する
        ...
        try{
            ...
            //List<Goal> Goal_some_achievement_list = dao.select_some_achievement_list(goal_id);//一部目標達成
            ...
            session.setAttribute("month_date",month_date); //目親標期間の情報をセット
            session.setAttribute("goal_id",goal_id); //目親標idの情報をセット            
            //session.setAttribute("Goal_some_achievement_list",Goal_some_achievement_list);//目標一部達成リスト設定
            /*for(Goal tmp : Goal_some_achievement_list){
                System.out.println("jsp前");
                Goal t = tmp;
                System.out.println(t.getgoal_month_child_id());
                System.out.println(t.getgoal());
            }*/
            Goal g = Goal_achievement_rate.get(0);
            String achievement_rate = g.get_achievement_rate();
            long miliseconds = System.currentTimeMillis();
            Date day = new Date(miliseconds);
            Boolean achievement_rate_update = update_dao.achievement_rate_update(goal_id,day,achievement_rate);//月間進捗表の進捗率をアップデート                            
