...
public class Goal_finish_month extends HttpServlet {
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        throws IOException, ServletException{
        HttpSession session = request.getSession();//セッション情報を取得。
        String goal_id = (String)session.getAttribute("goal_id");//セッション情報から取得
        String goal_month_child_id = request.getParameter("goal_month_child_id");//月間目標の子目標idを取得
        System.out.println("Goal_finish_monthきたよー");
        ...
        try{
            Boolean goal_achieve_update = update_dao.goal_achieve_update(goal_month_child_id);//月間子目標のアップデート
            if(goal_achieve_update){
                System.out.println("月間子目標進捗更新");
