...
public class Goal_select_year_DAO extends DAO{
    public List<Goal> select_achievement_parent_list(String goal_id) throws Exception{ //親目標達成リスト取得
        ...
        try{
              st=con.prepareStatement("select parent_id,goal from goal_year_parent where goal_id = ? and achieved_goals = 100"); //親目標の子目標全て達成している
    ...
    public List<Goal> select_achievement_child_list(String goal_id) throws Exception{ //子目標達成リスト取得
        ...
        try{
              //st=con.prepareStatement("select child_id,goal from goal_year_child where parent_id = ANY (select parent_id from goal_year_parent where goal_id = ? and achieved_goals = 100)"); //親目標の子目標全て達成している
              st=con.prepareStatement("select child_id,goal from goal_year_child where achieved_goal =1 and goal_id =?"); //親目標の子目標全て達成している
              st.setString(1,goal_id);
              ResultSet rs = st.executeQuery();
              while(rs.next()){
                  Goal g=new Goal();
    ...
    public List<Goal> select_not_achievement_parent_list(String goal_id) throws Exception{ //子目標未達成リスト取得
        ...
        try{
              //st=con.prepareStatement("select goal,parent_id from goal_year_child where parent_id = ANY (select parent_id from goal_year_parent where goal_id = ? and achieved_goals is null)"); //目標未達成の情報取得
              st=con.prepareStatement("select parent_id,goal from goal_year_parent where achieved_goals != 100 or achieved_goals is null and goal_id = ?"); //目標未達成の情報取得
              st.setString(1,goal_id);
              ResultSet rs = st.executeQuery();
              while(rs.next()){
                  Goal g=new Goal();
    ...
    return list;
     ...
     public List<Goal> select_not_achievement_child_list(String goal_id) throws Exception{ //子目標未達成リスト取得
        ...
        try{
              st=con.prepareStatement("select child_id,goal from goal_year_child where achieved_goal is null and goal_id = ?"); //目標未達成の情報取得
              st.setString(1,goal_id);
              ResultSet rs = st.executeQuery();
              while(rs.next()){
                  Goal g=new Goal();
    ...
    public List<Goal> select_goal_parent_all(String goal_id) throws Exception{ //Goal_createから月間の親目標を取得。
        ...
        try{
              st=con.prepareStatement("select * from goal_year_parent where goal_id=?"); //月間目標から目標を取得
              st.setString(1,goal_id);
              ResultSet rs = st.executeQuery();
              while(rs.next()){
                  Goal g=new Goal();
                  g.setgoal(rs.getString("goal"));//目標を設定
                  g.setgoal_year_parent_id(rs.getString("parent_id"));//親間目標idを設定
                  list.add(g);//リストに値を追加        
    ...
    public String check_parent(String goal_id){//親目標にデータがあるか確認
        try{
            ...
            st = con.prepareStatement("select parent_id from goal_year_parent where goal_id=?"); //親目標DBから親idを取得
            st.setString(1,goal_id);
            ResultSet rs_check_parent_id = st.executeQuery(); //sql文実施
            while(rs_check_parent_id.next()){
                Goal g = new Goal();//goalインスタンス生成
                g.setgoal_year_parent_id(rs_check_parent_id.getString("parent_id"));//取得した親目標idをgoalインスタンスに設定
    ...
    public List<Goal> select_goal_child(String parent_id) throws Exception{ //子目標取得。
        ...
        try{
              ...
              while(rs.next()){
                  ...
                  g.setgoal_year_child_id(rs.getString("child_id"));//子間目標idを設定
                  list.add(g);//リストに値を追加        
                  System.out.println(list);
            }
              st.close();
    ...
    public boolean check_child(String parent_id){//子目標を表示
        ...
        try{
            Connection con = getConnection(); //DBに接続
            PreparedStatement st;//目標削除。
            st = con.prepareStatement("select * from goal_year_child where parent_id=?"); //目標DBを削除。
            ...
            if (rs_check_child_id.next()){//子目標のidを取得
              ...
              g.setgoal_year_child_id(rs_check_child_id.getString("child_id"));
            }
